...
contract MusicianRenderer is IMusicianRenderer, Strings {
	function roleToString(Role role) public pure returns (string memory) {
		if (role == Role.VOCALIST) {
			return "Vocalist";
		} else if (role == Role.LEAD_GUITAR) {
			return "Lead Guitar";
		} else if (role == Role.RHYTHM_GUITAR) {
			return "Rhythm Guitar";
		} else if (role == Role.BASS) {
			return "Bass";
		} else if (role == Role.DRUMS) {
			return "Drums";
		} else {
			return "TBD";
		}
	...
	function constructTokenURI(Musician calldata musician, uint256 tokenId) external pure override returns (string memory) {
		...
		parts[4] = roleToString(musician.role);
